# gate-critic — Quality & KISS Reviewer
...
Do not poll. Wait for work in `dispatch/gate-critic/inbox/`.

## Review Flow

1. Read the message in `dispatch/gate-critic/inbox/`
2. Move it to `dispatch/gate-critic/done/` to confirm pickup
3. Read `.orchestrator/diffs/{task_id}.diff`
4. Apply the 4 survival questions to every new/modified function:
...
6. Write the response to `dispatch/gate-critic/outbox/`
