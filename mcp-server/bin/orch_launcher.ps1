...
function Invoke-DeployAgents {
    ...
    # Step 1: Pick source
    $source = Show-FolderBrowserDialog -Description "Select agent SOURCE folder (contains profiles/, protocols/)" -StartPath $defaultSource
    if (-not $source) { return }

    # Discover agent folders to deploy
    $profilesDir = Join-Path $source "profiles"
    ...
    if ($agentFolders.Count -eq 0) {
        Write-Host "  No agent profile folders found in: $profilesDir" -ForegroundColor Red
        Pause-Notice ""
        return
    }

    # Step 2: Checkbox to pick which agents
    Write-Host "  Found $($agentFolders.Count) agent(s) in source:" -ForegroundColor DarkGray
    $checkItems = @()
    foreach ($af in $agentFolders) {
        $checkItems += New-MenuItem -Label $af.Name -Value @{ name=$af.Name; checked=$true }
    }
    ...
    # Step 4: Deploy selected agents
    Write-Host ""
    Write-Host "  Deploying to: $target" -ForegroundColor Cyan
    $copied = 0
    foreach ($agentName in $selected) {
        # Deploy profiles
        $src = Join-Path $profilesDir $agentName
        if (Test-Path $src) {
            $dst = Join-Path $target "profiles\$agentName"
            if (-not (Test-Path $dst)) { New-Item -ItemType Directory -Path $dst -Force | Out-Null }
            Copy-Item -Path "$src\*" -Destination $dst -Recurse -Force
            Write-Host "    + profiles/$agentName/" -ForegroundColor Green
            $copied++
        }
        # Deploy protocols if they exist
        $srcP = Join-Path $protocolsDir $agentName
        if (Test-Path $srcP) {
            $dstP = Join-Path $target "protocols\$agentName"
            if (-not (Test-Path $dstP)) { New-Item -ItemType Directory -Path $dstP -Force | Out-Null }
            Copy-Item -Path "$srcP\*" -Destination $dstP -Recurse -Force
            Write-Host "    + protocols/$agentName/" -ForegroundColor Green
            $copied++
        }
    }

    Write-Host ""
    Write-Host "  Deployed $copied item(s) to $target" -ForegroundColor Green
    Pause-Notice ""
}
...
function Invoke-SprintPause {
    ...
    foreach ($agent in $Config.agents) {
        $a = $agent["name"]
        $agentRole = if ($agent.ContainsKey("role")) { $agent["role"] } else { "coder" }
        if ($agentRole -eq "coder") {
            ...
            Write-Host "    Skipped: $a ($agentRole)" -ForegroundColor DarkGray
        }
    }
