...
# Walk up from launcher location to find project root (.orchestrator/config.json)
$DEFAULT_PROJECT_ROOT = $null
$_search = $SCRIPT_DIR
while ($_search) {
    if (Test-Path (Join-Path $_search ".orchestrator/config.json")) {
        $DEFAULT_PROJECT_ROOT = $_search
        break
    }
    $_parent = Split-Path $_search -Parent
    if ($_parent -eq $_search) { break }
    $_search = $_parent
}
if (-not $DEFAULT_PROJECT_ROOT) {
    # Last resort: assume 2 levels up (dispatcher/bin/ -> project root)
    $DEFAULT_PROJECT_ROOT = (Resolve-Path (Join-Path $SCRIPT_DIR "..\..") -ErrorAction SilentlyContinue).Path
}
...
# Apply -ProjectRoot parameter, or walk-up from launcher location, or last-used state
if ([string]::IsNullOrWhiteSpace($ProjectRoot)) {
    if ($DEFAULT_PROJECT_ROOT) {
        # Prefer walk-up: launcher is inside a project
        $ProjectRoot = $DEFAULT_PROJECT_ROOT
    } else {
        # Fallback: last-used project from state file
...
# Engine root — where scripts/, hooks/, lib/ live. Walk up from launcher location.
# Does NOT change when user switches project via "Change Project Directory".
$EngineRoot = $null
$_er = $SCRIPT_DIR
while ($_er) {
    if (Test-Path (Join-Path $_er "scripts")) { $EngineRoot = $_er; break }
    $_p = Split-Path $_er -Parent
    if ($_p -eq $_er) { break }
    $_er = $_p
}
if (-not $EngineRoot) { $EngineRoot = $ProjectRoot }
...
function Read-ArrowMenu {
    ...
    if ($selectables.Count -eq 0) { return @{ action="cancel"; item=$null; selPos=0 } }
    ...
    while ($true) {
        ...
        switch ($k.Key) {
            ...
            'Enter'      { return @{ action="select"; item=$Items[$selectables[$selPos]]; selPos=$selPos } }
            'RightArrow' { return @{ action="select"; item=$Items[$selectables[$selPos]]; selPos=$selPos } }
            'Escape'     { return @{ action="cancel"; item=$null; selPos=$selPos } }
            'LeftArrow'  { return @{ action="back";   item=$null; selPos=$selPos } }
            default      { }
        }
    }
}
...
function Read-CheckboxMenu {
    ...
    # Extract display labels and keys — supports strings or objects with .Label/.Value
    $labels = @()
    $keys = @()
    foreach ($item in $Items) {
        if ($item -is [string]) {
            $labels += $item; $keys += $item
        } elseif ($item.Label) {
            $labels += [string]$item.Label; $keys += [string]$item.Label
        } else {
            $labels += $item.ToString(); $keys += $item.ToString()
        }
    }

    # Default all items to checked if not specified
    foreach ($key in $keys) {
        if (-not $Checked.ContainsKey($key)) { $Checked[$key] = $true }
    }
    ...
    while ($true) {
        $k = [Console]::ReadKey($true)
        switch ($k.Key) {
            'UpArrow'   { $selPos--; if ($selPos -lt 0) { $selPos = $keys.Count - 1 }; RenderCheckbox }
            'DownArrow' { $selPos++; if ($selPos -ge $keys.Count) { $selPos = 0 }; RenderCheckbox }
            'Spacebar'  { $Checked[$keys[$selPos]] = -not $Checked[$keys[$selPos]]; RenderCheckbox }
            'Enter'     { return @($Items | Where-Object { $lbl = if ($_ -is [string]) { $_ } elseif ($_.Label) { $_.Label } else { $_.ToString() }; $Checked[$lbl] }) }
            'Escape'    { return $null }
            default     { }
        }
    }
...
function Get-DashboardLines {
    ...
    # Sprint status from live roles.json
    $sprintLabel = "OFF"
    if (Test-Path $RolesJsonPath) {
        try {
            $liveRoles = Get-Content $RolesJsonPath -Raw -Encoding UTF8 | ConvertFrom-Json -AsHashtable
            if ($liveRoles.ContainsKey("hooks_config") -and $liveRoles["hooks_config"]["sprint_active"] -eq $true) {
                $sprintLabel = "ACTIVE"
            }
        } catch {}
    }
...
function Invoke-ClearHaltFlags {
    ...
    if (Test-Path $orchCtl) {
        ...
        if ($halts.Count -eq 0) {
            ...
            $halts | ForEach-Object { Remove-Item $_.FullName -Force }
...
function Get-HookFlagDir {
    $dir = Join-Path $RuntimeFlagsDir "hooks"
...
function Build-InstallMenu {
    @(
        ...
        (New-MenuItem -Label "22b. Deploy Agents..."          -Value @{ type="deploy_agents" }),
...
function Invoke-DeployToProject {
    ...
    $orchSubdirs = @("plans", "tasks", "diffs", "merged_verdicts", "halts", "runtime_flags", "runtime_flags/hooks", "logs", "approvals")
    ...
    # Find engine root: config shared_roots, then walk up from launcher
    $engineRoot = $null
    if ($Config -and $Config.shared_roots) {
        if ($Config.shared_roots.orchestrator_primary -and (Test-Path $Config.shared_roots.orchestrator_primary)) {
            $engineRoot = $Config.shared_roots.orchestrator_primary
        } elseif ($Config.shared_roots.orchestrator_fallback -and (Test-Path $Config.shared_roots.orchestrator_fallback)) {
    ...
    if ($engineRoot) {
        ...
        if (Test-Path $installScript) {
            Write-Host "    Running install_hooks.py --all --project $target ..." -ForegroundColor DarkGray
            $savedPythonPath = $env:PYTHONPATH
            $env:PYTHONPATH = $engineRoot
            Push-Location $engineRoot
            & python $installScript --all --project $target 2>&1 | ForEach-Object { Write-Host "    $_" }
            Pop-Location
            $env:PYTHONPATH = $savedPythonPath
            if ($LASTEXITCODE -eq 0) {
                Write-Host "    Hooks installed successfully." -ForegroundColor Green
            } else {
                Write-Host "    ! install_hooks.py exited with code $LASTEXITCODE" -ForegroundColor Yellow
            }
        } else {
...
function Invoke-DeployEngine {
    ...
    # dispatch/, docs/, memory/, logs/, .orchestrator/, .omc/, *.rar, CLAUDE.md, STATE.md
...
function Invoke-DeployAgents {
    # Default source: canonical agent repo
    $defaultSource = "D:\IA\dispatcher_repo\agents"
    if (-not (Test-Path $defaultSource)) { $defaultSource = Join-Path $ProjectRoot "agents" }
    # Default target: NAS shared agents
    $defaultTarget = "W:\Claude_Library\agents"

    Write-Host ""
    Write-Host "  DEPLOY AGENTS" -ForegroundColor Cyan
    Write-Host "  Select source agent folder, then target." -ForegroundColor DarkGray
    Write-Host ""

    # Step 1: Pick source
    $source = Show-FolderBrowserDialog -Description "Select agent SOURCE folder" -StartPath $defaultSource
    if (-not $source) { return }

    # Smart detection: find where agent folders actually are
    # Structure can be: agents/profiles/coordinated/gate-*/ or agents/profiles/standalone/*/
    # Or user may pick a subfolder directly. Walk the tree to find leaf agent folders.
    $profilesDir = $null
    $protocolsDir = $null

    # Check if picked folder has profiles/ subdir
    $candidateProfiles = Join-Path $source "profiles"
    if (Test-Path $candidateProfiles) {
        $profilesDir = $candidateProfiles
        $protocolsDir = Join-Path $source "protocols"
    } else {
        # User picked a folder that IS the profiles tree (or a subtree like profiles/coordinated)
        # Look for subdirs that contain .md files (agent folders)
        $profilesDir = $source
    }

    # Recursively find all agent folders (leaf dirs containing .md or CLAUDE.md files)
    $agentFolders = @()
    ...
    # Also check immediate children (non-recursive case)
    if ($agentFolders.Count -eq 0) {
        $agentFolders = @(Get-ChildItem -LiteralPath $profilesDir -Directory -ErrorAction SilentlyContinue)
    }

    if ($agentFolders.Count -eq 0) {
        Write-Host "  No agent folders found in: $profilesDir" -ForegroundColor Red
        Pause-Notice ""
        return
    }

    # Step 2: Checkbox to pick which agents
    Write-Host "  Found $($agentFolders.Count) agent(s):" -ForegroundColor DarkGray
    $checkItems = @()
    foreach ($af in $agentFolders) {
        # Show relative path from source for clarity
    ...
    # Step 3: Pick target
    $target = Show-FolderBrowserDialog -Description "Select agent TARGET folder" -StartPath $defaultTarget
    if (-not $target) { return }

    # Step 4: Deploy selected agents — preserve relative structure from source
    Write-Host ""
    Write-Host "  Deploying to: $target" -ForegroundColor Cyan
    $copied = 0
    foreach ($sel in $selected) {
        $srcPath = $sel.Value.fullPath
        $relPath = $sel.Value.relPath
    ...
    Write-Host ""
    Write-Host "  Deployed $copied agent(s) to $target" -ForegroundColor Green
    Pause-Notice ""
}
...
function Invoke-Doctor {
    ...
    $checks = @(
        ...
        @{ name="Watcher script exists"; ok=(Test-Path (Join-Path $EngineRoot "scripts\watcher.py")) },
        @{ name="orchestratorctl.py exists"; ok=(Test-Path (Join-Path $EngineRoot "scripts\orchestratorctl.py")) }
    )
...
function Start-WatcherWithSupervisor {
    # Clear leftover STOP flag from previous sprint so supervisor doesn't exit immediately
    $stopFile = Join-Path $RuntimeFlagsDir "STOP"
    if (Test-Path $stopFile) { Remove-Item $stopFile -Force }

    $watcherScript = Join-Path $EngineRoot "scripts\watcher.py"
...
function Invoke-SprintWizard {
    ...
    foreach ($a in $selectedAgents) {
        ...
        if ($mode -eq "skip") {
            Write-Host "    Skipping $a (existing session)" -ForegroundColor DarkGray
            continue
        }
...
:MAIN while ($true) {
    ...
    try {
        switch ($v.type) {
            ...
            "deploy_agents"        { Invoke-DeployAgents; continue }
